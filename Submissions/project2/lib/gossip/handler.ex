defmodule Gossip.Handler do
	def process(rumor,properties) do
		...
		cond do
			properties["algorithm"] == "gossip" ->
				...
				# IO.puts("L #{properties["id"]} #{properties["state"]}")
				properties
	...
	def send_rumor(properties) do
		# Sends rumor to random neighbour
		...
		if not Enum.empty?(properties["valid_neighbours"]) do
			...
			# IO.puts("S #{properties["id"]} #{properties["state"]} #{chosen_neighbour["id"]}")
			cond do
				properties["algorithm"] == "gossip" ->
					rumor = :ok
